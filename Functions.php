<?php 
 ...
 public function changePass($pass,$pass2){
	...
	$oneData=array();
	$stmt_login=$this->baglan->prepare("SELECT DISTINCT `country` FROM `states`");
	$stmt_login->execute();	
	$stmt_login->store_result();
	...
	$oneData=array();
	$stmt_login=$this->baglan->prepare("SELECT DISTINCT `city` FROM `states` WHERE country=?");
	$stmt_login->bind_param("s", $count);
	$stmt_login->execute();	
	$stmt_login->store_result();
	$stmt_login->bind_result($country);
	$numofrows=$stmt_login->num_rows;
	...
	public function getCountity($count, $city){
	$oneData=array();
	$stmt_login=$this->baglan->prepare("SELECT DISTINCT `countity` FROM `states` WHERE country=? AND city=?");
	$stmt_login->bind_param("ss", $count, $city);
	$stmt_login->execute();	
	$stmt_login->store_result();
	$stmt_login->bind_result($country);
	$numofrows=$stmt_login->num_rows;
